...
const { writeText, legacy, log } = require('@serverless/utils/log');

// TODO: Remove it with v3 and rename `writeMessageModern` to `writeMessage`
...
module.exports = async (exception, options = {}) => {
  ...
  const chalkSlsGrey = chalk.hex('#8c8d91');
  writeText([
    chalkSlsGrey(
    ...
    chalkSlsGrey('Docs:        docs.serverless.com'),
    chalkSlsGrey('Support:     forum.serverless.com'),
    chalkSlsGrey('Bugs:        github.com/serverless/serverless/issues'),
  ]);

  const errorMsg =
    exceptionTokens.stack && !isUserError ? exceptionTokens.stack : exceptionTokens.message;
  writeText(chalk.hex('#fd5750')('\nError:'));
  if (errorMsg) {
